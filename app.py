...
# generator
def generate_json_files(num_files, races: list, races_weights: list, classes: dict, classes_weights:list, weapons: dict, items:dict):
...
# generate the json files -- input the number of files to be generated
generate_json_files(99, races=races, races_weights=races_weights, classes=classes, classes_weights=classes_weights, weapons=weapons, items=items)

# generate the summary csv
folder_path = 'json' 
value_summary = generate_summary(folder_path)
output_file = 'summary.csv'

with open(output_file, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(['Value', 'Count'])
    for value, count in value_summary.items():
        writer.writerow([value, count])

folder_path = 'json' 
generate_images(folder_path)
